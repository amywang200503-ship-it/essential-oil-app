...
function QuoteManagement({ onBack, onCustomer, onProduct }: { onBack: () => void; onCustomer: () => void; onProduct: () => void }) {
  ...
  if (selected) return <QuoteDetail quote={selected} total={total(selected)} expiry={expiryText(selected)} onBack={() => setSelected(null)} onCustomer={onCustomer} onProduct={onProduct} onAction={showNotice} onSend={() => sendQuote(selected)} onAccept={() => acceptQuote(selected)} onConvertToOrder={() => convertToOrder(selected)} />
...
function QuoteForm({ onClose, onSubmit }: { onClose: () => void; onSubmit: (quote: Quote) => void }) { const { state } = useAppStore(); const [form, setForm] = useState({ customer: 'XX化妆品有限公司', product: '摩洛哥阿甘油', productId: state.products[0]?.id ?? '', batch: 'ARG20260801', quantity: '20', unit: 'KG', spec: '', price: '380', moq: '10', freight: '0', tax: '0', quoteDate: state.settings.simulatedToday, validUntil: '2026-08-30', payment: 'T/T 预付', delivery: '款到发货', note: '' }); const update = (key: string, value: string) => setForm((current) => ({ ...current, [key]: value })); const quantity = Number(form.quantity) || 0; const price = Number(form.price) || 0; const freight = Number(form.freight) || 0; const tax = Number(form.tax) || 0; const moq = Number(form.moq) || 0; const itemTotal = quantity * price; const productsWithStock = selectProductsWithStock(state); const currentProduct = productsWithStock.find((item) => item.id === form.productId); const stock = currentProduct?.currentStock ?? 0; const stockStatus = selectQuoteStockStatus(state, form.productId, form.unit); const save = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    ...
    onSubmit({ ...form, id: `Q20260823${String(Date.now()).slice(-3)}`, customerId, productId, product, contact: customerContact, quantity, price, moq, freight, tax, status: '草稿', stock, tone: 'sage' })
