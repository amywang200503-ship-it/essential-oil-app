...
function QuoteDetail({ quote, total, expiry, onBack, onCustomer, onProduct, onAction, onSend, onAccept, onConvertToOrder }: { quote: Quote; total: number; expiry: string; onBack: () => void; onCustomer: () => void; onProduct: () => void; onAction: (text: string) => void; onSend: () => void; onAccept: () => void; onConvertToOrder: () => void }) { const { state } = useAppStore(); const stockStatus = selectQuoteStockStatus(state, quote.productId, quote.unit); return <div className="quote-page quote-detail-page"><button className="back-link" onClick={onBack}>← 返回报价管理</button><div className="quote-detail-hero"><div className={`quote-mark ${quote.tone ?? ''}`}>¥</div><div><div className="eyebrow">报价档案 · {quote.id}</div><h1>{quote.id}</h1><p><button onClick={onCustomer}>{quote.customer}</button>　·　{quote.contact ?? ''}</p><div className="quote-tags"><span className={`quote-status ${quote.status === '已接受' ? 'accepted' : 'pending'}`}><i />{quote.status}</span>{expiry && <span className="expiry-tag">{expiry}</span>}</div></div><div className="quote-detail-buttons"><button className="secondary-button" onClick={() => onAction('报价已复制')}>复制报价</button><button className="primary-button" onClick={() => onSend()}>发送报价　→</button></div></div><div className="quote-detail-grid"><section className="quote-card"><div className="quote-card-head"><span className="section-kicker">01 · BASIC</span><h2>报价信息</h2></div><div className="quote-fields"><QuoteField label="报价编号" value={quote.id} /><QuoteField label="客户" value={quote.customer} clickable={onCustomer} /><QuoteField label="联系人" value={quote.contact ?? ''} /><QuoteField label="报价日期" value={quote.quoteDate} /><QuoteField label="有效期至" value={quote.validUntil} /><QuoteField label="报价状态" value={quote.status} /></div></section><section className="quote-card"><div className="quote-card-head"><span className="section-kicker">02 · PRODUCT</span><h2>产品明细</h2></div><div className="quote-product-detail"><div><b>{quote.product}</b><small>{quote.batch ?? ''} · {quote.spec ? `${quote.spec} · ` : ''}{quote.unit}</small></div><button onClick={onProduct}>查看产品详情 →</button></div><div className="quote-fields compact"><QuoteField label="数量" value={`${quote.quantity} ${quote.unit}`} /><QuoteField label="单价" value={`${money(quote.price)} / ${quote.unit}`} clickable={onProduct} /><QuoteField label="MOQ" value={`${quote.moq ?? 0} ${quote.unit}`} /><QuoteField label="商品金额" value={money(quote.quantity * quote.price)} /></div>{quote.quantity < (quote.moq ?? 0) && <div className="moq-warning">! 当前数量低于 MOQ，请确认采购数量。</div>}{stockStatus.comparable ? (quote.quantity > (stockStatus.stock ?? 0) && <div className="stock-warning">! 当前库存不足，无法满足本次报价数量，请确认采购计划。</div>) : <div className="stock-warning">{stockStatus.reason === 'unit-mismatch' ? `! 当前库存单位为 ${stockStatus.stockUnit}，与报价单位 ${quote.unit} 不一致，请先核对单位。` : `! 当前库存没有与报价单位 ${quote.unit} 一致的库存，暂不进行数量比较。`}</div>}</section><section className="quote-card amount-card"><div className="quote-card-head"><span className="section-kicker">03 · AMOUNT</span><h2>金额汇总</h2></div><div className="amount-lines"><p><span>商品金额</span><b>{money(quote.quantity * quote.price)}</b></p><p><span>运费</span><b>{money(quote.freight)}</b></p><p><span>税费</span><b>{money(quote.tax)}</b></p><p className="amount-total"><span>报价总金额</span><b>{money(total)}</b></p></div></section><section className="quote-card"><div className="quote-card-head"><span className="section-kicker">04 · TERMS</span><h2>交易条款</h2></div><div className="quote-fields compact"><QuoteField label="付款方式" value={quote.payment ?? '—'} /><QuoteField label="交货方式" value={quote.delivery ?? '—'} /><QuoteField label="备注" value={quote.note ?? '—'} /></div></section></div><div className="quote-next-actions"><b>下一步操作</b><button onClick={() => onAction('报价编辑已开启')}>编辑报价</button><button onClick={() => onAccept()}>客户接受</button><button onClick={() => onAction('报价已重新创建')}>重新报价</button><button onClick={() => onConvertToOrder()}>转为订单</button></div></div> }
