...
type InventoryItem = {
  ...
  outbound: number
  safety: number
  /** 产品当前批次的计量单位（多批次单位不一致时为 '多单位'，无 unit 的历史数据按 KG） */
  unit: string
  batch: string
  inboundDate: string
  outboundDate: string
...
const inventoryData: InventoryItem[] = [
  { name: '摩洛哥阿甘油', category: '植物油', inbound: 100, outbound: 28, safety: 10, unit: 'KG', batch: 'ARG20260801', inboundDate: '2026-08-01', outboundDate: '2026-08-20', expiry: '2028-08-01', tone: 'amber' },
  { name: '摩洛哥仙人掌籽油', category: '植物油', inbound: 20, outbound: 12, safety: 10, unit: 'KG', batch: 'CPS20260718', inboundDate: '2026-07-18', outboundDate: '2026-08-18', expiry: '2026-09-12', tone: 'sage' },
  { name: '摩洛哥熔岩泥', category: '天然泥粉', inbound: 150, outbound: 30, safety: 20, unit: 'KG', batch: 'RHL20260805', inboundDate: '2026-08-05', outboundDate: '2026-08-15', expiry: '2028-08-05', tone: 'clay' },
]
...
const mapInventoryItems = (inventory: StoreInventoryRecord[], products: StoreProduct[]): InventoryItem[] => {
  ...
  return Array.from(groups.values()).map((records) => {
    ...
    // 计量单位：该产品全部批次单位一致时取该单位，混合单位时标注“多单位”（不做跨单位换算）
    const units = Array.from(new Set(records.map((record) => recordUnit(record))))
    return {
      ...
      unit: units.length === 1 ? units[0] : '多单位',
      batch: latest.batch,
...
/**
 * FIFO：按入库日期升序返回某产品的批次（遗留 helper，行为与单位语义均未改变，内部委托给 selectors.getFifoInventoryRecords）。
 * 需要按计量单位过滤时请使用 getFifoInventoryRecords(inventory, productId, unit)。
 */
const fifoInboundRecords = (inventory: StoreInventoryRecord[], productId: string) => getFifoInventoryRecords(inventory, productId)

/** 该产品 FIFO 目标单位：最早入库批次的计量单位（未显式指定单位时作为本次扣减单位；无批次返回 undefined）。 */
const fifoHeadUnit = (inventory: StoreInventoryRecord[], productId: string) => {
  const head = fifoInboundRecords(inventory, productId)[0]
  return head ? recordUnit(head) : undefined
}
...
function OrderManagement({ onBack, onCustomer, onProduct }: { onBack: () => void; onCustomer: () => void; onProduct: () => void }) {
  ...
  const visible = orders.filter((order) => { const text = `${order.id} ${order.customer} ${order.contact ?? ''} ${order.product} ${order.tracking ?? ''}`.toLowerCase(); return text.includes(query.toLowerCase()) && (filter === '全部' || order.status === filter) })
  const saveOrder = (order: Order) => {
    if (!order.quantity || order.quantity <= 0) return
    // 单位一致性：只从与订单单位相同的批次预占（不同单位不能直接比较，也不能直接占用）
    const orderUnit = order.unit || DEFAULT_INVENTORY_UNIT
    const { allocations: reservations, remaining } = planFifoAllocation(state.inventory, order.productId, order.quantity, orderUnit)
    if (remaining > 0) {
      showNotice(`${orderUnit} 库存不足，无法确认订单（缺 ${formatInventoryAmount(remaining, orderUnit)}，其它单位库存不可替代）`)
      return
    }
    reservations.forEach((reservation) => dispatch({ type: 'RESERVE_STOCK', payload: { inventoryId: reservation.inventoryId, quantity: reservation.quantity, unit: orderUnit } }))
    dispatch({ type: 'CREATE_ORDER', payload: { ...order, reservations } })
    setFormOpen(false)
    showNotice(`订单已创建，库存已按 FIFO 预占 ${formatInventoryAmount(order.quantity, orderUnit)}`)
  }
  ...
  const confirmShipping = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // 单位一致性：预占批次的单位必须与订单单位一致（不同单位不能直接扣减）
    const orderUnit = selected.unit || DEFAULT_INVENTORY_UNIT
    const mismatch = findReservationUnitMismatch(state.inventory, selected.reservations ?? [], orderUnit)
    if (mismatch) {
      showNotice(`批次 ${mismatch.batch} 单位为 ${mismatch.unit}，与订单单位 ${orderUnit} 不一致，无法发货`)
      return
    }
    ...
    consumption.forEach((item) => {
      dispatch({ type: 'RELEASE_STOCK', payload: { inventoryId: item.inventoryId, quantity: item.quantity, unit: orderUnit } })
      // 出库记录来源：订单号（referenceId）+ 来源报价号（note），便于库存流水追溯
      dispatch({ type: 'STOCK_OUT', payload: { inventoryId: item.inventoryId, quantity: item.quantity, unit: orderUnit, referenceId: selected.orderNo ?? selected.id, note: selected.quoteId ? `来源报价 ${selected.quoteId}` : `来源订单 ${selected.orderNo ?? selected.id}` } })
    })
  ...
  const advanceOrderStatus = (order: Order) => {
    ...
    // 备货中 → 已发货：不直接改状态，改为打开发货确认界面（第一阶段仅确认发货状态，不扣减库存）
    if (live.status === '备货中') {
  ...
  const cancelOrder = (order: Order) => {
    ...
    // 释放预占：单位取该预占批次自身的单位（兼容历史遗留的跨单位预占，避免预占无法释放）
  ...
  const buildRelinkPlan = (order: Order) => {
    ...
    // 与订单创建完全相同的单位过滤规则：补关联同样不允许跨单位补足
    const orderUnit = order.unit || DEFAULT_INVENTORY_UNIT
    const { allocations: reservations, remaining: need } = planFifoAllocation(state.inventory, order.productId, remaining, orderUnit)
    return { remaining, reservations, insufficient: need > 0, unit: orderUnit }
  }
  const confirmRelink = () => {
    if (!selected) return
    const plan = buildRelinkPlan(selected)
    if (plan.insufficient) {
      showNotice(`${plan.unit} 库存不足，无法完成库存补关联（缺 ${formatInventoryAmount(plan.remaining - plan.reservations.reduce((sum, item) => sum + item.quantity, 0), plan.unit)}）`)
      return
    }
    plan.reservations.forEach((reservation) => dispatch({ type: 'RESERVE_STOCK', payload: { inventoryId: reservation.inventoryId, quantity: reservation.quantity, unit: plan.unit } }))
...
function OrderForm({ onClose, onSubmit }: { onClose: () => void; onSubmit: (order: Order) => void }) { const { state } = useAppStore(); const today = state.settings.simulatedToday; const [form, setForm] = useState({ customer: state.customers[0]?.company ?? '', contact: '李女士', product: '摩洛哥阿甘油', productId: state.products[0]?.id ?? '', batch: 'ARG20260801', quantity: '20', unit: 'KG', price: '380', freight: '0', tax: '0', payment: 'T/T 预付', delivery: '款到发货', expected: '2026-08-30', note: '' }); const update = (key: string, value: string) => setForm((current) => ({ ...current, [key]: value })); const productsWithStock = selectProductsWithStock(state); const currentProduct = productsWithStock.find((item) => item.id === form.productId); const quantity = Number(form.quantity) || 0; const price = Number(form.price) || 0; const freight = Number(form.freight) || 0; const tax = Number(form.tax) || 0; const stock = currentProduct?.currentStock ?? 0; const save = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); const id = `SO${today.replace(/-/g, '')}${String(Date.now()).slice(-3)}`; const customerId = state.customers.find((item) => item.company === form.customer)?.id ?? ''; const productId = form.productId; const productName = state.products.find((item) => item.id === productId)?.name ?? form.product; onSubmit({ id, orderNo: id, customerId, customer: form.customer, contact: form.contact, quoteId: '', quote: '手动创建', productId, product: productName, batch: form.batch, quantity, shipped: 0, unit: form.unit, price, freight, tax, status: '待付款', payment: '待付款', paid: 0, orderDate: today, delivery: form.delivery, expected: form.expected, logistics: '', tracking: '', stock, tone: 'sage', createdAt: today, updatedAt: today, totalAmount: quantity * price + freight + tax, paidAmount: 0, items: [{ productId, productName, quantity, unit: form.unit, price, amount: quantity * price, shippedQuantity: 0 }], timeline: [{ date: today, event: '创建订单' }] }) }; return <div className="modal-backdrop"><form className="order-form" onSubmit={save}><div className="form-head"><div><span className="section-kicker">NEW ORDER</span><h2>新建订单</h2><p>订单创建后先预占库存，发货时才扣减实际库存。</p></div><button type="button" onClick={onClose}>×</button></div><div className="form-body"><div className="form-grid"><label className="form-field"><span>客户<i>*</i></span><select value={form.customer} onChange={(event) => update('customer', event.target.value)}>{Array.from(new Set([...state.customers.map((item) => item.company), form.customer].filter(Boolean))).map((company) => <option key={company}>{company}</option>)}</select></label><label className="form-field"><span>联系人</span><input value={form.contact} onChange={(event) => update('contact', event.target.value)} /></label><label className="form-field"><span>产品<i>*</i></span><select value={form.productId} onChange={(event) => { const productId = event.target.value; update('productId', productId); const product = state.products.find((item) => item.id === productId); if (product) { update('product', product.name); update('price', String(product.price)) } }}>{state.products.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label><label className="form-field"><span>批次</span><input value={form.batch} onChange={(event) => update('batch', event.target.value)} /></label><label className="form-field"><span>数量<i>*</i></span><input type="number" min="0.01" step="0.01" value={form.quantity} onChange={(event) => update('quantity', event.target.value)} /></label><label className="form-field"><span>单位</span><select value={form.unit} onChange={(event) => update('unit', event.target.value)}><option>KG</option><option>ML</option></select></label><label className="form-field"><span>单价<i>*</i></span><input type="number" min="0" value={form.price} onChange={(event) => update('price', event.target.value)} /></label><label className="form-field"><span>运费</span><input type="number" min="0" value={form.freight} onChange={(event) => update('freight', event.target.value)} /></label><label className="form-field"><span>税费</span><input type="number" min="0" value={form.tax} onChange={(event) => update('tax', event.target.value)} /></label><label className="form-field"><span>付款方式</span><select value={form.payment} onChange={(event) => update('payment', event.target.value)}><option>T/T 预付</option><option>月结</option></select></label><label className="form-field"><span>交货方式</span><select value={form.delivery} onChange={(event) => update('delivery', event.target.value)}><option>款到发货</option><option>工厂交货</option></select></label><label className="form-field"><span>预计交付日期</span><input type="date" value={form.expected} onChange={(event) => update('expected', event.target.value)} /></label></div><div className="order-calculator"><div><span>商品金额</span><b>{money(quantity * price)}</b></div><div><span>订单总金额</span><b>{money(quantity * price + freight + tax)}</b></div><div><span>预占库存</span><b>{quantity} {form.unit}</b></div></div>{quantity > stock && <div className="stock-warning">! 库存不足，无法确认订单。当前可用库存 {stock} {form.unit}。</div>}<label className="form-field full-field"><span>备注</span><textarea value={form.note} onChange={(event) => update('note', event.target.value)} placeholder="补充订单要求..." /></label></div><div className="form-foot"><button type="button" className="cancel-button" onClick={onClose}>取消</button><button type="submit" className="primary-button" disabled={quantity > stock}>保存订单　→</button></div></form></div> }

function RelinkForm({ order, plan, onClose, onConfirm }: { order: Order; plan: { remaining: number; reservations: { inventoryId: string; batch: string; quantity: number }[]; insufficient: boolean; unit: string }; onClose: () => void; onConfirm: () => void }) { return <div className="modal-backdrop"><form className="small-form" onSubmit={(event) => { event.preventDefault(); onConfirm() }}><div className="form-head"><div><span className="section-kicker">STOCK RELINK</span><h2>库存补关联</h2><p>该订单未建立库存预占，发货前需补关联库存。</p></div><button type="button" onClick={onClose}>×</button></div><div className="form-body"><div className="batch-detail-list"><p><span>订单号</span><b>{order.id}</b></p><p><span>产品</span><b>{order.product}</b></p><p><span>订单数量</span><b>{order.quantity} {order.unit}</b></p><p><span>已发货</span><b>{order.shipped} {order.unit}</b></p><p><span>待补关联数量</span><b>{plan.remaining} {order.unit}</b></p></div>{plan.insufficient ? <div className="stock-warning">{plan.unit} 库存不足，无法补关联（缺 {formatInventoryAmount(plan.remaining, plan.unit)}）。</div> : <div className="batch-detail-list">{plan.reservations.map((reservation) => <p key={reservation.inventoryId}><span>批次 {reservation.batch}</span><b>{formatInventoryAmount(reservation.quantity, plan.unit)}</b></p>)}</div>}</div><div className="form-foot"><button type="button" className="cancel-button" onClick={onClose}>取消</button><button type="submit" className="primary-button" disabled={plan.insufficient}>确认补关联　→</button></div></form></div> }

function ShippingForm({ onClose, onSubmit, max, unit }: { onClose: () => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void; max: number; unit?: string }) { const { state } = useAppStore(); return <div className="modal-backdrop"><form className="small-form" onSubmit={onSubmit}><div className="form-head"><div><span className="section-kicker">SHIPMENT</span><h2>确认发货</h2><p>确认后将从实际库存扣减发货数量。</p></div><button type="button" onClick={onClose}>×</button></div><div className="form-body"><label className="form-field"><span>发货数量（最多 {formatInventoryAmount(max, unit)}）<i>*</i></span><input name="amount" type="number" min="0.01" max={max} step="0.01" defaultValue={max} required /></label><label className="form-field"><span>物流公司</span><input name="logistics" placeholder="如：顺丰速运" /></label><label className="form-field"><span>物流单号</span><input name="tracking" placeholder="请输入物流单号" /></label><label className="form-field"><span>发货日期</span><input type="date" defaultValue={state.settings.simulatedToday} /></label></div><div className="form-foot"><button type="button" className="cancel-button" onClick={onClose}>取消</button><button type="submit" className="primary-button">确认发货　→</button></div></form></div> }

function PaymentForm({ onClose, onSubmit, total }: { onClose: () => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void; total: number }) { const { state } = useAppStore(); return <div className="modal-backdrop"><form className="small-form" onSubmit={onSubmit}><div className="form-head"><div><span className="section-kicker">PAYMENT</span><h2>记录付款</h2><p>待收款金额 {money(total)}。</p></div><button type="button" onClick={onClose}>×</button></div><div className="form-body"><label className="form-field"><span>付款金额<i>*</i></span><input name="amount" type="number" min="0.01" max={total} step="0.01" defaultValue={total} required /></label><label className="form-field"><span>付款日期</span><input name="date" type="date" defaultValue={state.settings.simulatedToday} /></label><label className="form-field"><span>付款备注</span><textarea name="note" placeholder="补充付款信息..." /></label></div><div className="form-foot"><button type="button" className="cancel-button" onClick={onClose}>取消</button><button type="submit" className="primary-button">保存付款　→</button></div></form></div> }
...
const quoteSentStatuses = ['已发送', '客户查看', '待确认']

type FormulaIngredient = { id: number; productId: string; name: string; category: string; ratio: number; unit: string; price: number; stock: number; tone: string }
...
const toStoreFormula = (formula: Formula): StoreFormula => ({
  id: formula.id,
  name: formula.name,
...
function AiAssistant({ onBack, navigate }: { onBack: () => void; navigate: (target: string) => void }) {
  ...
  const tasks = [
    ...
    ...(lowStock.slice(0, 1).map((item) => { const unit = selectProductUnit(state, item.productId); return { title: `检查 ${item.productName} 库存`, desc: `${formatInventoryAmount(item.inbound - item.outbound, unit)} 低于安全库存 ${formatInventoryAmount(item.safetyStock, unit)}`, priority: '高', action: '库存管理' } })),
...
function DataCenter({ onBack, navigate }: { onBack: () => void; navigate: (target: string) => void }) {
  ...
  // 平均订单金额：与「销售额 / 成交订单」统一口径（仅已完成订单；已取消/待付款/备货中/已发货不计入）
  const averageOrder = orderCount ? sales / orderCount : 0
  ...
  // 单位感知库存展示：总量按单位分列（旧函数 selectTotalInventory / selectAvailableStock 保持不变，兼容历史数据）
  const inventoryTotalLabel = selectInventoryTotalLabel(state)
  ...
  // 低库存产品按其批次单位展示（无 unit 的历史数据回退 KG）
  const lowStockUnit = selectProductUnit(state, lowStockFirst?.productId ?? '')
...
type DocumentItem = { id: string; name: string; type: string; productId?: string; product: string; batch: string; version: string; publish: string; expiry: string; status: string; tone: string }
...
function ProductDocuments({ onBack }: { onBack: () => void }) {
  ...
  const [query, setQuery] = useState(''); const [filter, setFilter] = useState('全部'); const [productId, setProductId] = useState<string | null>(null); const [showForm, setShowForm] = useState(false); const [sendList, setSendList] = useState<string[]>([]); const [notice, setNotice] = useState('')
  ...
  const completeness = (targetProductId: string) => { const found = documents.filter((doc) => doc.productId === targetProductId).map((doc) => doc.type); return Math.round(requiredDocs.filter((type) => found.includes(type)).length / requiredDocs.length * 100) }
  const incompleteProducts = state.products.filter((p) => completeness(p.id) < 100)
  const completeProducts = state.products.filter((p) => completeness(p.id) === 100)
  const saveDocument = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); const productId = String(data.get('product')); const productRecord = state.products.find((item) => item.id === productId); if (!productRecord) { notify('未找到该产品，请先在产品中心创建产品'); return } const newDoc: DocumentItem = { id: `DOC-${state.productDocuments.length + 1}`, name: String(data.get('name')), type: String(data.get('type')), productId, product: productRecord.name, batch: String(data.get('batch') || '-'), version: String(data.get('version') || 'V1.0'), publish: String(data.get('publish') || state.settings.simulatedToday), expiry: String(data.get('expiry') || ''), status: '已上传', tone: 'sage' }; dispatch({ type: 'ADD_DOCUMENT', payload: { id: newDoc.id, name: newDoc.name, type: newDoc.type, productId: newDoc.productId ?? '', product: newDoc.product, batch: newDoc.batch, version: newDoc.version, publish: newDoc.publish, expiry: newDoc.expiry, status: newDoc.status } }); setDocuments((current) => [newDoc, ...current]); setShowForm(false); notify('资料已添加') }
  if (productId) return <DocumentProductDetail productId={productId} documents={documents} completeness={completeness(productId)} onBack={() => setProductId(null)} sendList={sendList} setSendList={setSendList} notify={notify} />
  return <div className="document-page"><div className="document-head"><div><button className="back-link" onClick={onBack}>← 返回工作台</button><div className="eyebrow">资料资产 · PRODUCT DOCUMENTS</div><h1>产品资料</h1><p>统一管理产品技术文件、批次资料和客户资料。</p></div><button className="primary-button" onClick={() => setShowForm(true)}>＋ 新增资料</button></div><div className="document-metrics"><div><span>产品总数</span><b>{state.products.length}</b><em>已建立资料档案</em></div><div><span>资料完整</span><b>{completeProducts.length}</b><em>资料齐全产品</em></div><div className="document-warm"><span>资料待完善</span><b>{incompleteProducts.length}</b><em>需要补齐文件</em></div><div className="document-alert"><span>即将过期</span><b>{documents.filter(expiring).length}</b><em>30天内需要关注</em></div><div><span>批次资料</span><b>{documents.filter((doc) => doc.type === '批次资料').length}</b><em>关联具体批次</em></div></div><div className="document-advice"><span>✦</span><div><b>资料提醒</b><small>有 {incompleteProducts.length} 个产品资料尚未完善。　有 {documents.filter(expiring).length} 份 COA 将在30天内到期。　{incompleteProducts[0] ? `${incompleteProducts[0].name}目前资料完整度${completeness(incompleteProducts[0].id)}%` : '所有产品资料均已完善'}。</small></div><button onClick={() => setFilter('即将到期')}>查看提醒 →</button></div><div className="document-toolbar"><label className="catalog-search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索产品、INCI、批次、文件名称..." /></label><div className="document-tabs">{documentTypes.map((item) => <button className={filter === item ? 'document-tab active' : 'document-tab'} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div></div><div className="document-summary"><span>资料列表 <b>{visible.length}</b></span><span>暂无真实文件上传，当前为模拟资料</span></div><div className="document-table"><div className="document-table-head"><span>文件名称</span><span>资料类型</span><span>关联产品</span><span>关联批次</span><span>版本</span><span>发布日期</span><span>有效期</span><span>操作</span></div>{visible.map((doc) => <article className="document-row" key={doc.id}><div className="document-name"><div className={`document-icon ${doc.tone}`}>▤</div><div><b>{doc.name}</b><small>{doc.id}</small></div></div><span className="document-type">{doc.type}</span><button className="document-product" onClick={() => setProductId(doc.productId ?? '')}>{doc.product}　↗</button><span className="document-batch">{doc.batch}</span><span className="document-version">{doc.version}</span><span className="document-date">{doc.publish}</span><span className={`document-expiry ${expiring(doc) || expired(doc) ? 'warning' : ''}`}>{doc.expiry || '长期有效'}{expiring(doc) && <small>即将到期</small>}{expired(doc) && <small>已过期</small>}</span><div className="document-actions"><button onClick={() => setProductId(doc.productId ?? '')}>查看</button><button onClick={() => notify('资料编辑已开启')}>编辑</button><button onClick={() => { dispatch({ type: 'DELETE_DOCUMENT', payload: { id: doc.id } }); setDocuments((current) => current.filter((item) => item.id !== doc.id)) }}>删除</button></div></article>)}</div>{showForm && <DocumentForm onClose={() => setShowForm(false)} onSubmit={saveDocument} />}{notice && <div className="toast">✓ {notice}</div>}</div>
}

function DocumentProductDetail({ productId, documents, completeness, onBack, sendList, setSendList, notify }: { productId: string; documents: DocumentItem[]; completeness: number; onBack: () => void; sendList: string[]; setSendList: (items: string[]) => void; notify: (text: string) => void }) { const { state } = useAppStore(); const productRecord = state.products.find((p) => p.id === productId); const productDocs = documents.filter((doc) => doc.productId === productId); const types = ['COA', 'SDS', 'TDS', '规格书', '检测报告', '原产地资料', '批次资料', '图片', '标签']; const toggle = (type: string) => setSendList(sendList.includes(type) ? sendList.filter((item) => item !== type) : [...sendList, type]); return <div className="document-page document-detail-page"><button className="back-link" onClick={onBack}>← 返回产品资料</button><div className="document-detail-hero"><div className="document-product-mark">▤</div><div><div className="eyebrow">产品资料档案</div><h1>{productRecord?.name ?? productId}</h1><p>{productRecord?.en ?? '—'}</p><div className="document-tags"><span>{productRecord?.category ?? '—'}</span><span>{productRecord?.origin ?? '—'}</span></div></div><div className="completeness"><b>{completeness}%</b><span>资料完整度</span></div></div><div className="document-detail-layout"><section className="document-card"><div className="document-card-head"><span className="section-kicker">01 · PROFILE</span><h2>产品资料概览</h2></div><div className="document-fields"><DocField label="产品名称" value={productRecord?.name ?? productId} /><DocField label="英文名称" value={productRecord?.en ?? '—'} /><DocField label="INCI" value={productRecord?.inci ?? '—'} /><DocField label="产品分类" value={productRecord?.category ?? '—'} /><DocField label="产地" value={productRecord?.origin ?? '—'} /><DocField label="资料完整度" value={`${completeness}% · ${completeness === 100 ? '资料完整' : '部分资料'}`} /></div></section><section className="document-card"><div className="document-card-head"><span className="section-kicker">02 · BATCH</span><h2>关联批次资料</h2></div><div className="batch-document-list">{productDocs.filter((doc) => doc.batch !== '-').map((doc) => <button key={doc.id} onClick={() => notify(`已查看 ${doc.batch} 相关资料`)}><span><b>{doc.type}</b> {doc.name}</span><small>{doc.batch}　→</small></button>)}<button onClick={() => notify('批次资料入口已打开')}><span><b>批次资料</b> 查看关联批次</span><small>{productRecord?.batch ?? '—'}　→</small></button></div></section></div><section className="document-card document-status-card"><div className="document-card-head"><span className="section-kicker">03 · DOCUMENT STATUS</span><h2>资料清单</h2><p>缺少的资料会影响客户审核和批次放行。</p></div><div className="document-checklist">{types.map((type) => { const doc = productDocs.find((item) => item.type === type); const status = doc ? (doc.expiry && new Date(doc.expiry) < new Date(state.settings.simulatedToday) ? '已过期' : doc.expiry && (new Date(doc.expiry).getTime() - new Date(state.settings.simulatedToday).getTime()) / 86400000 <= 30 ? '即将到期' : '已上传') : '未完善'; return <div key={type}><span><i className={status === '已上传' ? 'good' : status === '未完善' ? 'missing' : 'warning'} />{type}</span><b>{status}</b><button onClick={() => toggle(type)} className={sendList.includes(type) ? 'selected' : ''}>{sendList.includes(type) ? '已选择' : '发送资料'}</button></div> })}</div><div className="send-summary"><span>已选择 <b>{sendList.length}</b> 份资料</span><button className="primary-button" onClick={() => notify(`资料已加入发送清单，共 ${sendList.length} 份`)}>发送资料 →</button></div></section></div> }

function DocField({ label, value }: { label: string; value: string }) { return <div><small>{label}</small><b>{value}</b></div> }
function DocumentForm({ onClose, onSubmit }: { onClose: () => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void }) { const { state } = useAppStore(); return <div className="modal-backdrop"><form className="document-form" onSubmit={onSubmit}><div className="form-head"><div><span className="section-kicker">NEW DOCUMENT</span><h2>新增资料</h2><p>暂不上传真实文件，先建立资料索引。</p></div><button type="button" onClick={onClose}>×</button></div><div className="form-body"><div className="form-grid"><label className="form-field"><span>资料名称<i>*</i></span><input name="name" required placeholder="如：阿甘油检测报告" /></label><label className="form-field"><span>资料类型<i>*</i></span><select name="type"><option>COA</option><option>SDS</option><option>TDS</option><option>规格书</option><option>检测报告</option><option>原产地资料</option><option>批次资料</option></select></label><label className="form-field"><span>关联产品<i>*</i></span><select name="product">{state.products.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="form-field"><span>关联批次</span><input name="batch" placeholder="如：ARG20260801" /></label><label className="form-field"><span>文件编号</span><input name="fileNo" placeholder="如：COA-20260801" /></label><label className="form-field"><span>版本号</span><input name="version" defaultValue="V1.0" /></label><label className="form-field"><span>发布日期</span><input name="publish" type="date" defaultValue={state.settings.simulatedToday} /></label><label className="form-field"><span>有效期</span><input name="expiry" type="date" /></label><label className="form-field"><span>资料状态</span><select name="status"><option>已上传</option><option>待完善</option></select></label></div><label className="form-field full-field"><span>备注</span><textarea name="note" placeholder="补充资料说明..." /></label></div><div className="form-foot"><button type="button" className="cancel-button" onClick={onClose}>取消</button><button className="primary-button" type="submit">保存资料　→</button></div></form></div> }
function FormulaLab({ onBack }: { onBack: () => void }) {
  ...
  const saveFormula = (formula: Formula) => { const storeFormula = toStoreFormula(formula); const exists = formulas.some((item) => item.id === formula.id); if (exists) dispatch({ type: 'UPDATE_FORMULA', payload: { id: formula.id, changes: storeFormula } }); else dispatch({ type: 'ADD_FORMULA', payload: storeFormula }); setFormulas((current) => exists ? current.map((item) => item.id === formula.id ? formula : item) : [formula, ...current]); setEditing(null); setShowForm(false); notify('配方已保存') }
  const copyFormula = (formula: Formula) => { const copy = { ...formula, id: `F${state.settings.simulatedToday.replace(/-/g, '')}${String(formulas.length + 1).padStart(3, '0')}`, name: `${formula.name}（副本）`, status: '草稿', version: 'V1.0', ingredients: formula.ingredients.map((item) => ({ ...item })) }; dispatch({ type: 'ADD_FORMULA', payload: toStoreFormula(copy) }); setFormulas((current) => [copy, ...current]); notify('已生成新的配方草稿') }
...
function FormulaEditor({ formula, onBack, onSave, onCopy, onDelete, notify }: { formula: Formula | null; onBack: () => void; onSave: (formula: Formula) => void; onCopy: () => void; onDelete: () => void; notify: (text: string) => void }) { const { state } = useAppStore();
  ...
  const [ingredients, setIngredients] = useState<FormulaIngredient[]>(formula?.ingredients ?? state.products.slice(0, 4).map((product, index) => ({ id: index + 1, productId: product.id, name: product.name, category: product.category, ratio: [60, 30, 8, 2][index] ?? 0, unit: 'g', price: product.price, stock: stockFromInventory(state.inventory, product.id), tone: product.tone })))
  ...
  const addIngredient = (event: React.ChangeEvent<HTMLSelectElement>) => { const product = state.products.find((candidate) => candidate.id === event.target.value); if (product && !ingredients.some((current) => current.name === product.name)) setIngredients((current) => [...current, { id: Date.now(), productId: product.id, name: product.name, category: product.category, ratio: 0, unit: 'g', price: product.price, stock: stockFromInventory(state.inventory, product.id), tone: product.tone }]); event.target.value = '' }
...
function QuoteManagement({ onBack, onCustomer, onProduct }: { onBack: () => void; onCustomer: () => void; onProduct: () => void }) {
  ...
  const [showForm, setShowForm] = useState(false)
  const [renewSource, setRenewSource] = useState<Quote | null>(null)
  const [editTarget, setEditTarget] = useState<Quote | null>(null)
  const [notice, setNotice] = useState('')
  const showNotice = (text: string) => { setNotice(text); window.setTimeout(() => setNotice(''), 2300) }
  const daysToExpiry = (quote: Quote) => quoteExpiryState(quote.validUntil, today).days
  const expiryText = (quote: Quote) => quoteExpiryState(quote.validUntil, today).label
  const isExpired = (quote: Quote) => quoteExpiryState(quote.validUntil, today).expired
  const visible = quotes.filter((quote) => { const text = `${quote.id} ${quote.customer} ${quote.contact} ${quote.product}`.toLowerCase(); const matchesFilter = filter === '全部' || (filter === '已过期' ? isExpired(quote) : quote.status === filter); return text.includes(query.toLowerCase()) && matchesFilter })
  const total = (quote: Quote) => quote.quantity * quote.price + quote.freight + quote.tax
  const saveQuote = (quote: Quote, openDetail = false) => { dispatch({ type: 'ADD_QUOTE', payload: quote }); setShowForm(false); setRenewSource(null); if (openDetail) setSelected(quote); showNotice(`报价 ${quote.id} 已创建`) }
  /** 重新报价：以原报价为模板打开新建表单，原报价保持原样（不改状态、不覆盖、不删除）。 */
  const renewQuote = (quote: Quote) => { setSelected(null); setRenewSource(quote); showNotice(`已复制报价 ${quote.id}，请确认有效期后保存为新报价`) }
  /** 编辑报价：先退出报价详情视图（否则会被详情分支的提前 return 挡住），再以原报价为模板打开编辑表单（编号保持不变）。 */
  const editQuote = (quote: Quote) => { setSelected(null); setEditTarget(quote); showNotice(`编辑报价 ${quote.id}（保存后编号保持不变）`) }
  /** 编辑保存：按原报价 ID 更新原报价本身，保留其原状态与色调；不新增报价、不建订单、不动库存。 */
  const updateQuote = (quote: Quote) => { const updated = { ...quote, status: editTarget?.status ?? quote.status, tone: editTarget?.tone ?? quote.tone }; dispatch({ type: 'UPDATE_QUOTE', payload: { id: quote.id, changes: updated } }); setEditTarget(null); setSelected(updated); showNotice(`报价 ${quote.id} 已更新`) }
  /** 重新报价用的模板：沿用原报价内容，但换成当前有效日期与新的有效期（不复制已过期日期）。 */
  const renewSeed = renewSource ? { ...renewSource, id: '', quoteDate: resolveEffectiveToday(today), validUntil: quoteDefaultValidUntil(today), status: '草稿' } : null
  /** 客户确认：仅「已发出、等待客户确认」的报价可确认；只改状态，保留 id 与其它字段（不新增报价、不动库存）。 */
  const acceptQuote = (quote: Quote) => {
    if (!quoteSentStatuses.includes(quote.status)) {
      showNotice(`报价当前状态为「${quote.status}」，请先发送报价，再由客户确认`)
      return
    }
    dispatch({ type: 'UPDATE_QUOTE', payload: { id: quote.id, changes: { status: '已接受' } } })
    setSelected({ ...quote, status: '已接受' })
    showNotice('报价已接受，状态已更新')
  }
  const convertToOrder = (quote: Quote) => {
    // 拦截一：同一报价不能重复转单（按已有订单的 quoteId / quote 判定）
    const existingOrder = state.orders.find((order) => order.quoteId === quote.id || order.quote === quote.id)
    if (existingOrder) {
      showNotice(`该报价已转为订单 ${existingOrder.orderNo ?? existingOrder.id}，不能重复转单`)
      return
    }
    // 拦截二：报价有效期已过（有效期至 < 基准今天）时不允许转单；历史报价日期不做修改
    const expiry = quoteExpiryState(quote.validUntil, today)
    if (expiry.expired) {
      showNotice(`报价已过期（有效期至 ${quote.validUntil}），不能转为订单；请重新报价或延长有效期`)
      return
    }
    // 拦截三：报价需先被客户接受
    if (quote.status !== '已接受') {
      showNotice(`报价当前状态为「${quote.status}」，需先点击“客户接受”后才能转为订单`)
      return
    }
    if (!isValidBusinessUnit(quote.unit)) {
      showNotice(`报价单位「${quote.unit}」不是有效计量单位（仅支持 ${BUSINESS_UNITS.join(' / ')}），请先修正单位后再转订单`)
      return
    }
    // 订单编号独立生成（SO + 日期 + 时间片），绝不使用产品批次号或报价编号
    const id = `SO${today.replace(/-/g, '')}${String(Date.now()).slice(-5)}`
    dispatch({ type: 'CREATE_ORDER', payload: { id, orderNo: id, customerId: quote.customerId, customer: quote.customer, quoteId: quote.id, quoteNo: quote.id, quote: quote.id, productId: quote.productId, product: quote.product, batch: quote.batch ?? '', quantity: quote.quantity, shipped: 0, unit: quote.unit, price: quote.price, freight: quote.freight, tax: quote.tax, status: '待付款', payment: '待付款', paid: 0, orderDate: today, contact: quote.contact ?? '', delivery: quote.delivery ?? '款到发货', expected: quote.validUntil, logistics: '', tracking: '', stock: quote.stock ?? 0, tone: quote.tone ?? 'sage', createdAt: today, updatedAt: today, totalAmount: total(quote), paidAmount: 0, items: [{ productId: quote.productId, productName: quote.product, quantity: quote.quantity, unit: quote.unit, price: quote.price, amount: quote.quantity * quote.price, shippedQuantity: 0 }], timeline: [{ date: today, event: '客户接受报价' }, { date: today, event: `由报价 ${quote.id} 创建订单` }] } })
    dispatch({ type: 'UPDATE_QUOTE', payload: { id: quote.id, changes: { status: '已转订单' } } })
    setSelected({ ...quote, status: '已转订单' })
    showNotice(`已转为订单 ${id}（来源报价 ${quote.id}）`)
  }
  const sendQuote = (quote: Quote) => {
    if (quote.status !== '草稿') {
      showNotice(`报价当前状态为「${quote.status}」，无需重复发送`)
      return
    }
    dispatch({ type: 'UPDATE_QUOTE', payload: { id: quote.id, changes: { status: '已发送' } } })
    setSelected({ ...quote, status: '已发送' })
    showNotice('报价已发送，状态已更新为已发送')
  }
  if (selected) return <><QuoteDetail quote={selected} total={total(selected)} expiry={expiryText(selected)} onBack={() => setSelected(null)} onCustomer={onCustomer} onProduct={onProduct} onSend={() => sendQuote(selected)} onAccept={() => acceptQuote(selected)} onConvertToOrder={() => convertToOrder(selected)} onRenew={() => renewQuote(selected)} onEdit={() => editQuote(selected)} />{notice && <div className="toast">✓ {notice}</div>}</>
  const waitingCount = quotes.filter((q) => ['已发送', '客户查看', '待确认'].includes(q.status)).length
  return <div className="quote-page"><div className="quote-head"><div><button className="back-link" onClick={onBack}>← 返回工作台</button><div className="eyebrow">商业资产 · QUOTE MANAGEMENT</div><h1>报价管理</h1><p>统一管理报价、有效期和客户反馈。</p></div><button className="primary-button" onClick={() => setShowForm(true)}>＋ 新建报价</button></div><div className="quote-metrics"><div><span>本月报价</span><b>{quotes.length}</b><em>本月已创建</em></div><div><span>待客户确认</span><b>{waitingCount}</b><em>等待客户反馈</em></div><div className="quote-warm"><span>即将到期</span><b>{quotes.filter((q) => daysToExpiry(q) >= 0 && daysToExpiry(q) <= 7).length}</b><em>7天内需要关注</em></div><div className="quote-done"><span>已成交</span><b>{quotes.filter((q) => ['已接受', '已转订单'].includes(q.status)).length}</b><em>已接受报价</em></div><div className="quote-alert"><span>已失效</span><b>{quotes.filter((q) => daysToExpiry(q) < 0 || q.status === '已拒绝').length}</b><em>需要重新报价</em></div></div><div className="quote-advice"><span>✦</span><div><b>报价提醒</b><small>有 {waitingCount} 份报价等待客户确认。　有 {quotes.filter((q) => daysToExpiry(q) >= 0 && daysToExpiry(q) <= 7).length} 份报价将在7天内到期。</small></div><button onClick={() => setFilter('待确认')}>查看待确认 →</button></div><div className="quote-toolbar"><label className="catalog-search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索报价编号、客户、产品、联系人..." /></label><div className="quote-tabs">{quoteFilters.map((item) => <button className={filter === item ? 'quote-tab active' : 'quote-tab'} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div></div><div className="quote-summary"><span>报价列表 <b>{visible.length}</b></span><span>模拟日期：{today}</span></div><div className="quote-table"><div className="quote-table-head"><span>报价单</span><span>客户</span><span>产品明细</span><span>数量 / 单价</span><span>总金额</span><span>有效期</span><span>状态</span><span>操作</span></div>{visible.length ? visible.map((quote) => <article className="quote-row" key={quote.id}><div className="quote-id"><b>{quote.id}</b><small>{quote.quoteDate}</small></div><div className="quote-customer"><b>{quote.customer}</b><small>{quote.contact ?? ''}</small></div><div className="quote-product"><b>{quote.product}</b><small>{quote.batch ?? ''}</small></div><div className="quote-quantity"><b>{quote.quantity} {quote.unit}</b><small>{money(quote.price)} / {quote.unit}</small></div><strong className="quote-total">{money(total(quote))}</strong><div className={`quote-expiry ${isExpired(quote) ? 'expired' : expiryText(quote) ? 'expiring' : ''}`}><span>{quote.validUntil}</span>{expiryText(quote) && <small>{expiryText(quote)}</small>}</div><span className={`quote-status ${quote.status === '已接受' ? 'accepted' : quote.status === '待确认' ? 'pending' : 'sent'}`}><i />{quote.status}</span><div className="quote-actions"><button onClick={() => setSelected(quote)}>查看详情　→</button><button onClick={() => renewQuote(quote)}>复制报价</button></div></article>) : <div className="empty-catalog"><span>⌕</span><b>没有找到匹配的报价</b><small>试试报价编号、客户或产品关键词</small></div>}</div>{showForm && <QuoteForm onClose={() => setShowForm(false)} onSubmit={(quote) => saveQuote(quote)} />}{renewSeed && <QuoteForm initial={renewSeed} onClose={() => setRenewSource(null)} onSubmit={(quote) => saveQuote(quote, true)} />}{editTarget && <QuoteForm initial={editTarget} keepId onClose={() => setEditTarget(null)} onSubmit={updateQuote} />}{notice && <div className="toast">✓ {notice}</div>}</div>
}

function QuoteDetail({ quote, total, expiry, onBack, onCustomer, onProduct, onSend, onAccept, onConvertToOrder, onRenew, onEdit }: { quote: Quote; total: number; expiry: string; onBack: () => void; onCustomer: () => void; onProduct: () => void; onSend: () => void; onAccept: () => void; onConvertToOrder: () => void; onRenew: () => void; onEdit: () => void }) { const { state } = useAppStore(); const stockStatus = selectQuoteStockStatus(state, quote.productId, quote.unit); const expired = quoteExpiryState(quote.validUntil, state.settings.simulatedToday).expired; const convertedOrder = state.orders.find((order) => order.quoteId === quote.id || order.quote === quote.id); const convertDisabled = expired || Boolean(convertedOrder); const convertHint = convertedOrder ? `该报价已转为订单 ${convertedOrder.orderNo ?? convertedOrder.id}` : '报价已过期，请重新报价'; const sendDisabled = expired || quote.status !== '草稿'; const sendHint = expired ? '报价已过期，请重新报价' : `报价当前状态为「${quote.status}」，无需重复发送`; const statusAction: { label: string; onClick?: () => void; disabled: boolean; hint: string } | null = quote.status === '草稿' ? { label: '发送报价　→', onClick: onSend, disabled: sendDisabled, hint: sendDisabled ? sendHint : '发送报价' } : quoteSentStatuses.includes(quote.status) ? { label: '客户已确认　→', onClick: onAccept, disabled: false, hint: '客户已确认报价' } : quote.status === '已接受' ? { label: '生成订单　→', onClick: onConvertToOrder, disabled: convertDisabled, hint: convertDisabled ? convertHint : '生成订单' } : null; return <div className="quote-page quote-detail-page"><button className="back-link" onClick={onBack}>← 返回报价管理</button><div className="quote-detail-hero"><div className={`quote-mark ${quote.tone ?? ''}`}>¥</div><div><div className="eyebrow">报价档案 · {quote.id}</div><h1>{quote.id}</h1><p><button onClick={onCustomer}>{quote.customer}</button>　·　{quote.contact ?? ''}</p><div className="quote-tags"><span className={`quote-status ${quote.status === '已接受' ? 'accepted' : 'pending'}`}><i />{quote.status}</span>{expiry && <span className={`expiry-tag${expired ? ' expired' : ''}`}>{expiry}</span>}{convertedOrder && <span className="expiry-tag">已转订单 {convertedOrder.orderNo ?? convertedOrder.id}</span>}</div></div><div className="quote-detail-buttons"><button className="secondary-button" onClick={() => onRenew()}>复制报价</button>{statusAction && <button className="primary-button" disabled={statusAction.disabled} title={statusAction.hint} onClick={statusAction.onClick}>{statusAction.label}</button>}</div></div><div className="quote-detail-grid"><section className="quote-card"><div className="quote-card-head"><span className="section-kicker">01 · BASIC</span><h2>报价信息</h2></div><div className="quote-fields"><QuoteField label="报价编号" value={quote.id} /><QuoteField label="客户" value={quote.customer} clickable={onCustomer} /><QuoteField label="联系人" value={quote.contact ?? ''} /><QuoteField label="报价日期" value={quote.quoteDate} /><QuoteField label="有效期至" value={quote.validUntil} /><QuoteField label="报价状态" value={quote.status} /></div></section><section className="quote-card"><div className="quote-card-head"><span className="section-kicker">02 · PRODUCT</span><h2>产品明细</h2></div><div className="quote-product-detail"><div><b>{quote.product}</b><small>{quote.batch ?? ''} · {quote.spec ? `${quote.spec} · ` : ''}{quote.unit}</small></div><button onClick={onProduct}>查看产品详情 →</button></div><div className="quote-fields compact"><QuoteField label="数量" value={`${quote.quantity} ${quote.unit}`} /><QuoteField label="单价" value={`${money(quote.price)} / ${quote.unit}`} clickable={onProduct} /><QuoteField label="MOQ" value={`${quote.moq ?? 0} ${quote.unit}`} /><QuoteField label="商品金额" value={money(quote.quantity * quote.price)} /></div>{quote.quantity < (quote.moq ?? 0) && <div className="moq-warning">! 当前数量低于 MOQ，请确认采购数量。</div>}{stockStatus.comparable ? (quote.quantity > (stockStatus.stock ?? 0) && <div className="stock-warning">! 当前库存不足，无法满足本次报价数量，请确认采购计划。</div>) : <div className="stock-warning">{stockStatus.reason === 'unit-mismatch' ? `! 当前库存单位为 ${stockStatus.stockUnit}，与报价单位 ${quote.unit} 不一致，请先核对单位。` : `! 当前库存没有与报价单位 ${quote.unit} 一致的库存，暂不进行数量比较。`}</div>}</section><section className="quote-card amount-card"><div className="quote-card-head"><span className="section-kicker">03 · AMOUNT</span><h2>金额汇总</h2></div><div className="amount-lines"><p><span>商品金额</span><b>{money(quote.quantity * quote.price)}</b></p><p><span>运费</span><b>{money(quote.freight)}</b></p><p><span>税费</span><b>{money(quote.tax)}</b></p><p className="amount-total"><span>报价总金额</span><b>{money(total)}</b></p></div></section><section className="quote-card"><div className="quote-card-head"><span className="section-kicker">04 · TERMS</span><h2>交易条款</h2></div><div className="quote-fields compact"><QuoteField label="付款方式" value={quote.payment ?? '—'} /><QuoteField label="交货方式" value={quote.delivery ?? '—'} /><QuoteField label="备注" value={quote.note ?? '—'} /></div></section></div><div className="quote-next-actions"><b>下一步操作</b><button onClick={() => onEdit()}>编辑报价</button><button onClick={() => onRenew()}>重新报价</button></div></div> }
...
function QuoteForm({ onClose, onSubmit, initial, keepId }: { onClose: () => void; onSubmit: (quote: Quote) => void; initial?: Quote | null; keepId?: boolean }) { const { state } = useAppStore(); const effectiveToday = resolveEffectiveToday(state.settings.simulatedToday); const defaultValidUntil = quoteDefaultValidUntil(state.settings.simulatedToday); const [form, setForm] = useState(() => initial ? { customer: initial.customer, contact: initial.contact ?? '', product: initial.product, productId: initial.productId, batch: initial.batch ?? '', quantity: String(initial.quantity ?? ''), unit: initial.unit, spec: initial.spec ?? '', price: String(initial.price ?? ''), moq: String(initial.moq ?? ''), freight: String(initial.freight ?? 0), tax: String(initial.tax ?? 0), quoteDate: initial.quoteDate || effectiveToday, validUntil: initial.validUntil || defaultValidUntil, payment: initial.payment ?? 'T/T 预付', delivery: initial.delivery ?? '款到发货', note: initial.note ?? '' } : { customer: state.customers[0]?.company ?? '', contact: '', product: '摩洛哥阿甘油', productId: state.products[0]?.id ?? '', batch: 'ARG20260801', quantity: '20', unit: 'KG', spec: '', price: '380', moq: '10', freight: '0', tax: '0', quoteDate: effectiveToday, validUntil: defaultValidUntil, payment: 'T/T 预付', delivery: '款到发货', note: '' }); const update = (key: string, value: string) => setForm((current) => ({ ...current, [key]: value })); const quantity = Number(form.quantity) || 0; const price = Number(form.price) || 0; const freight = Number(form.freight) || 0; const tax = Number(form.tax) || 0; const moq = Number(form.moq) || 0; const itemTotal = quantity * price; const productsWithStock = selectProductsWithStock(state); const currentProduct = productsWithStock.find((item) => item.id === form.productId); const stock = currentProduct?.currentStock ?? 0; const stockStatus = selectQuoteStockStatus(state, form.productId, form.unit); const save = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    const customerId = state.customers.find((item) => item.company === form.customer)?.id ?? ''
    const productId = form.productId
    const product = state.products.find((item) => item.id === productId)?.name ?? form.product
    const customerContact = state.customers.find((item) => item.company === form.customer)?.contact ?? '待补充'
    onSubmit({ ...form, id: keepId && initial?.id ? initial.id : nextQuoteNo(state.quotes, state.settings.simulatedToday), customerId, productId, product, contact: form.contact || customerContact, quantity, price, moq, freight, tax, status: '草稿', stock, tone: 'sage' })
  }; return <div className="modal-backdrop"><form className="quote-form" onSubmit={save}><div className="form-head"><div><span className="section-kicker">{keepId ? 'EDIT QUOTE' : 'NEW QUOTE'}</span><h2>{keepId ? '编辑报价' : '新建报价'}</h2><p>{keepId ? '修改后保存将更新原报价，报价编号保持不变。' : '报价金额会随数量、单价和费用自动计算。'}</p></div><button type="button" onClick={onClose}>×</button></div><div className="form-body"><div className="form-grid"><label className="form-field"><span>客户<i>*</i></span><select value={form.customer} onChange={(event) => update('customer', event.target.value)}>{Array.from(new Set([...state.customers.map((item) => item.company), form.customer].filter(Boolean))).map((company) => <option key={company}>{company}</option>)}</select></label><label className="form-field"><span>联系人</span><input value={form.contact} onChange={(event) => update('contact', event.target.value)} placeholder="留空则取客户联系人" /></label><label className="form-field"><span>报价日期<i>*</i></span><input type="date" value={form.quoteDate} onChange={(event) => update('quoteDate', event.target.value)} /></label><label className="form-field"><span>有效期<i>*</i></span><input type="date" value={form.validUntil} onChange={(event) => update('validUntil', event.target.value)} /></label><label className="form-field"><span>产品<i>*</i></span><select value={form.productId} onChange={(event) => { const productId = event.target.value; update('productId', productId); const product = state.products.find((item) => item.id === productId); if (product) { update('product', product.name); update('price', String(product.price)) } const productUnit = selectProductUnit(state, productId); if (productUnit !== '多单位') update('unit', productUnit); const headRecord = getFifoInventoryRecords(state.inventory, productId)[0]; update('batch', headRecord ? headRecord.batch : '') }}>{state.products.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label><label className="form-field"><span>批次</span><input value={form.batch} onChange={(event) => update('batch', event.target.value)} /></label><label className="form-field"><span>包装规格</span><input value={form.spec} onChange={(event) => update('spec', event.target.value)} placeholder="如：1 KG / 25 KG" /></label><label className="form-field"><span>数量<i>*</i></span><input type="number" min="0" step="0.01" value={form.quantity} onChange={(event) => update('quantity', event.target.value)} /></label><label className="form-field"><span>单位</span><select value={form.unit} onChange={(event) => update('unit', event.target.value)}>{BUSINESS_UNITS.map((unit) => <option key={unit}>{unit}</option>)}</select></label><label className="form-field"><span>单价<i>*</i></span><input type="number" min="0" value={form.price} onChange={(event) => update('price', event.target.value)} /></label><label className="form-field"><span>MOQ</span><input type="number" min="0" value={form.moq} onChange={(event) => update('moq', event.target.value)} /></label><label className="form-field"><span>运费</span><input type="number" min="0" value={form.freight} onChange={(event) => update('freight', event.target.value)} /></label><label className="form-field"><span>税费</span><input type="number" min="0" value={form.tax} onChange={(event) => update('tax', event.target.value)} /></label><label className="form-field"><span>付款方式</span><select value={form.payment} onChange={(event) => update('payment', event.target.value)}><option>T/T 预付</option><option>月结</option><option>款到发货</option></select></label><label className="form-field"><span>交货方式</span><select value={form.delivery} onChange={(event) => update('delivery', event.target.value)}><option>款到发货</option><option>工厂交货</option><option>快递到付</option></select></label></div><div className="quote-calculator"><div><span>商品金额</span><b>{money(itemTotal)}</b></div><div><span>报价总金额</span><b>{money(itemTotal + freight + tax)}</b></div></div>{quantity < moq && <div className="moq-warning">! 当前数量低于 MOQ，请确认采购数量。</div>}{stockStatus.comparable ? (quantity > (stockStatus.stock ?? 0) && <div className="stock-warning">! 当前库存不足，无法满足本次报价数量，请确认采购计划。</div>) : <div className="stock-warning">{stockStatus.reason === 'unit-mismatch' ? `! 当前库存单位为 ${stockStatus.stockUnit}，与报价单位 ${form.unit} 不一致，请先核对单位。` : `! 当前库存没有与报价单位 ${form.unit} 一致的库存，暂不进行数量比较。`}</div>}<label className="form-field full-field"><span>备注</span><textarea value={form.note} onChange={(event) => update('note', event.target.value)} placeholder="补充报价条件或客户需求..." /></label></div><div className="form-foot"><button type="button" className="cancel-button" onClick={onClose}>取消</button><button type="submit" className="primary-button">保存报价　→</button></div></form></div> }
...
function SampleManagement({ onBack }: { onBack: () => void }) {
  ...
  const changeStatus = (sample: Sample, status: string, event: string) => {
    let deductedUnit: string | undefined
    if (status === '已寄出' && sample.status !== '已寄出') {
      // 单位一致性：指定批次时以该批次单位为准；未指定批次时以最早入库批次的单位为目标单位，绝不跨单位寻找库存
      ...
      if (sample.batch && !exactMatch) {
        showNotice(`批次 ${sample.batch} 不存在，无法寄出样品`)
        return
      }
      deductedUnit = exactMatch ? recordUnit(exactMatch) : fifoHeadUnit(state.inventory, sample.productId)
      const unitRecords = getFifoInventoryRecords(state.inventory, sample.productId, deductedUnit)
      const record = exactMatch
        ?? unitRecords.find((item) => item.inbound - item.outbound - item.reserved >= sample.quantity)
        ?? unitRecords.find((item) => item.inbound - item.outbound - item.reserved > 0)
      if (!record || !deductedUnit) {
      ...
      if (record.inbound - record.outbound - record.reserved < sample.quantity) {
        showNotice(`批次 ${record.batch} 可用库存不足（可用 ${formatInventoryAmount(record.inbound - record.outbound - record.reserved, deductedUnit)}，已预占 ${formatInventoryAmount(record.reserved, deductedUnit)}），无法寄出样品`)
        return
      }
      dispatch({ type: 'STOCK_OUT', payload: { inventoryId: record.id, quantity: sample.quantity, unit: deductedUnit } })
    }
    ...
    setSelected(updated)
    showNotice(status === '已寄出' ? `样品已寄出，库存扣减 ${formatInventoryAmount(sample.quantity, deductedUnit ?? fifoHeadUnit(state.inventory, sample.productId))}` : `状态已更新为${status}`)
  }
  ...
  const saveSample = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const productName = state.products.find((item) => item.id === productId)?.name ?? ''
...
function InventoryManagement({ onBack }: { onBack: () => void }) {
  ...
  const totalLabel = selectInventoryTotalLabel(state)
  ...
  const applyTransaction = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    const data = new FormData(event.currentTarget)
    const product = state.products.find((item) => item.id === String(data.get('product')))
    const amount = Number(data.get('amount')) || 0
    if (!product) {
      showNotice('未找到该产品，请先在产品中心创建产品')
      return
    }
    if (amount <= 0) return
    ...
    // 计量单位与采购成本：出库表单无 unit 字段，缺失时按默认单位 KG
    ...
    if (transaction === 'inbound') {
      if (existingBatch) {
        dispatch({ type: 'STOCK_IN', payload: { inventoryId: existingBatch.id, quantity: amount, date: date || undefined, transactionId: transactionId || undefined, unit, unitCost: cost || undefined } })
      } else if (productId) {
        dispatch({
          ...
          payload: {
            productId,
            productName: name,
            category: product.category ?? productRecords[0]?.category ?? '',
            batch,
            inbound: amount,
      ...
      setTransaction(null)
      showNotice(`已入库 ${formatInventoryAmount(amount, unit)}，库存已更新`)
      return
    }
    if (transaction === 'outbound') {
      ...
      // 单位一致性：指定批次时用该批次自己的单位；FIFO 出库时以最早入库批次的单位为目标单位，不跨单位混扣
      const targetUnit = existingBatch ? recordUnit(existingBatch) : fifoHeadUnit(state.inventory, productId)
      if (!targetUnit) {
        showNotice('未找到可出库的库存')
        return
      }
      const fifoRecords = getFifoInventoryRecords(state.inventory, productId, targetUnit)
      ...
      if (!record) {
        showNotice(`未找到可出库的库存（${targetUnit} 单位下无可用批次）`)
        return
      }
      const recordAvailable = record.inbound - record.outbound - record.reserved
      if (recordAvailable < amount) {
        showNotice(`批次 ${record.batch} 可用库存不足，当前可用 ${formatInventoryAmount(recordAvailable, targetUnit)}（已预占 ${formatInventoryAmount(record.reserved, targetUnit)}）`)
        return
      }
      dispatch({ type: 'STOCK_OUT', payload: { inventoryId: record.id, quantity: amount, date: date || undefined, transactionId: transactionId || undefined, unit: targetUnit } })
      setTransaction(null)
      showNotice(`已出库 ${formatInventoryAmount(amount, targetUnit)}，库存已更新`)
    }
  }
  const saveSafety = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (safetyItem) {
      const productId = safetyItem.productId
...
function BatchDetail({ item, batches, stock, onClose }: { item: InventoryItem; batches: StoreInventoryRecord[]; stock: number; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="batch-detail"><div className="form-head"><div><span className="section-kicker">BATCH RECORDS</span><h2>{item.name}</h2><p>{item.category} · 总库存 {formatInventoryAmount(stock, item.unit)} · 共 {batches.length} 个批次</p></div><button onClick={onClose}>×</button></div>{batches.map((record) => { const current = batchStock(record); const status = batchStatus(record); return <div key={record.id}><div className="batch-numbers"><div><span>批次号</span><b>{record.batch}</b></div><div><span>当前库存</span><b className={current < record.safetyStock ? 'warning-number' : ''}>{current} <small>{recordUnit(record)}</small></b></div><div><span>状态</span><b>{status}</b></div></div><div className="batch-detail-list"><p><span>入库数量</span><b>{formatInventoryAmount(record.inbound, recordUnit(record))}</b></p><p><span>已出库</span><b>{formatInventoryAmount(record.outbound, recordUnit(record))}</b></p><p><span>入库日期</span><b>{record.inboundDate}</b></p><p><span>最近出库</span><b>{record.outboundDate || '—'}</b></p><p><span>到期日期</span><b>{record.expiry || '—'}</b></p><p><span>安全库存</span><b>{formatInventoryAmount(record.safetyStock, recordUnit(record))}</b></p></div></div> })}<button className="primary-button batch-close" onClick={onClose}>完成　✓</button></section></div>
}
...
function ProductCenter({ onBack }: { onBack: () => void }) {
  ...
  const saveProduct = (event: React.FormEvent) => {
    ...
    if (state.products.some((product) => product.name === form.name && product.id !== editingId)) {
      setNotice('产品名称已存在，请使用不同名称。')
...
function App() {
  ...
  const pendingPayments = selectPendingPayments(state)   // 已取消订单已在 selector 口径中排除
  ...
  pendingTasks.lowStock.forEach((item) => { const unit = selectProductUnit(state, item.productId); todoCards.push({ key: `low-${item.id}`, priority: 'low', icon: '↘', statusText: '库存提醒', time: '需要关注', title: item.productName, desc: `当前库存：${formatInventoryAmount(item.inbound - item.outbound, unit)} · 安全库存：${formatInventoryAmount(item.safetyStock, unit)}`, small: '建议及时补充库存。', action: '查看库存', target: '库存管理' }) })
  ...
  pendingShipments.forEach((order) => todoCards.push({ key: `ship-${order.id}`, priority: 'medium', icon: '↗', statusText: '待发货', time: '安排物流', title: order.customer, desc: `订单 ${order.id} · 已发 ${order.shipped} / ${order.quantity} ${order.unit}`, small: '请安排发货。', action: '查看订单', target: '订单管理' }))
