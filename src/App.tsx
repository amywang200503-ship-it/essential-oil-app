...
function OrderManagement({ onBack, onCustomer, onProduct }: { onBack: () => void; onCustomer: () => void; onProduct: () => void }) {
  ...
  const available = (order: Order) => (order.stock ?? 0) - order.quantity + order.shipped
...
type AnalyticsPeriod = '今日' | '本周' | '本月' | '本季度' | '今年' | '自定义'
function DataCenter({ onBack, navigate }: { onBack: () => void; navigate: (target: string) => void }) {
  ...
  const sales = selectMonthlySales(state)
  const orderCount = state.orders.filter((order) => isSameMonth(order.orderDate, today)).length
  const customerTotal = customers.length
  const completed = selectCompletedOrders(state).length
  const conversion = customerTotal ? ((completed / customerTotal) * 100).toFixed(1) : '0.0'
  const funnel = selectSalesFunnel(state)
  ...
  const sampleQuoteRate = sampleCount ? ((quoteCount / sampleCount) * 100).toFixed(1) : '0.0'
  const sampleOrderRate = sampleCount ? ((completed / sampleCount) * 100).toFixed(1) : '0.0'
  const funnelRate = (value: number) => customerTotal ? ((value / customerTotal) * 100).toFixed(1) : '0.0'
  ...
  const productAnalysis = state.products.map((product) => {
    const quoteN = state.quotes.filter((quote) => quote.productId === product.id).length
    const sampleN = state.samples.filter((sample) => sample.productId === product.id).length
    const orderN = state.orders.filter((order) => order.productId === product.id && order.quoteId).length
    const doneN = state.orders.filter((order) => order.productId === product.id && order.status === '已完成').length
    const qty = state.orders.filter((order) => order.productId === product.id).reduce((sum, order) => sum + order.quantity, 0)
    const amount = state.orders.filter((order) => order.productId === product.id && order.status === '已完成').reduce((sum, order) => sum + order.quantity * order.price + order.freight + order.tax, 0)
    return [product.name, quoteN, sampleN, orderN, doneN, `${qty} KG`, money(amount)] as [string, number, number, number, number, string, string]
  })
  const topCustomers = customers.map((customer) => ({ customer, stats: selectCustomerStats(state, customer.id) })).sort((a, b) => b.stats.totalSpent - a.stats.totalSpent).slice(0, 5)
  const topProducts = state.products.map((product) => {
    const orders = state.orders.filter((order) => order.productId === product.id)
    const quantity = orders.reduce((sum, order) => sum + order.quantity, 0)
    const sales = orders.filter((order) => order.status === '已完成').reduce((sum, order) => sum + order.quantity * order.price + order.freight + order.tax, 0)
    return { id: product.id, name: product.name, quantity, sales, orderCount: orders.length }
  ...
  return <div className="analytics-page"><div className="analytics-head"><div><button className="back-link" onClick={onBack}>← 返回工作台</button><div className="eyebrow">经营分析 · BUSINESS INTELLIGENCE</div><h1>数据中心</h1><p>从客户、产品、库存和订单中发现业务机会。</p></div><div className="analytics-periods">{(['今日', '本周', '本月', '本季度', '今年', '自定义'] as AnalyticsPeriod[]).map((item) => <button className={period === item ? 'active' : ''} key={item} onClick={() => setPeriod(item)}>{item}</button>)}</div></div><div className="analytics-snapshot"><span>当前视图：{period}</span><small>数据更新于 {state.settings.simulatedToday}</small></div><section className="analytics-metrics"><Metric label="本月销售额" value={money(sales)} change="↑ 12.8%" tone="warm" /><Metric label="本月订单数" value={String(orderCount)} change="↑ 4.2%" /><Metric label="新增客户" value={String(customerTotal)} change="↑ 18.5%" /><Metric label="成交客户" value={String(completed)} change={`${conversion}% 转化率`} /><Metric label="报价数量" value={String(quoteCount)} change="↑ 9.1%" /><Metric label="样品数量" value={String(sampleCount)} change="6 份等待反馈" /><Metric label="客户转化率" value={`${conversion}%`} change="客户 → 成交" tone="green" /><Metric label="平均订单金额" value={money(averageOrder)} change="本月均值" /></section><div className="analytics-advice"><span>✦</span><div><b>业务建议</b><p>摩洛哥仙人掌籽油近期询价增加，但库存低于安全库存，建议关注补货。</p><p>XX化妆品有限公司近期连续询价，建议优先跟进。</p></div><button onClick={() => navigate('库存管理')}>查看库存 →</button></div><div className="analytics-grid"><section className="analytics-card sales-chart-card"><CardTitle kicker="01 · SALES" title="销售趋势" action={<select defaultValue="销售额"><option>销售额</option><option>订单数量</option></select>} /><div className="chart-legend"><span><i />销售额</span><b>{money(salesTrendTotal)} <small>最近7天</small></b></div><div className="analytics-chart"><div className="analytics-y">{[salesMax, salesMax * 0.75, salesMax * 0.5, salesMax * 0.25, 0].map((value, index) => <span key={index}>{value.toFixed(0)}</span>)}</div><div className="analytics-chart-area"><div className="analytics-grid-lines"><i /><i /><i /><i /><i /></div><svg viewBox="0 0 600 190" preserveAspectRatio="none" aria-label="销售趋势图"><path d={salesAreaPath} fill="#b8c8a244" /><path d={salesLinePath} fill="none" stroke="#6f8a59" strokeWidth="3" /></svg><div className="analytics-x">{dailySales.map((item) => <span key={item.date}>{item.date.slice(5)}</span>)}</div></div></div></section><section className="analytics-card funnel-card"><CardTitle kicker="02 · FUNNEL" title="销售漏斗" /><div className="funnel">{[['客户', funnel.customers, 'funnel-one'], ['询价', funnel.inquiries, 'funnel-two'], ['样品', funnel.samples, 'funnel-three'], ['报价', funnel.quotes, 'funnel-four'], ['成交', funnel.completedOrders, 'funnel-five']].map(([label, value, tone], index) => <div key={label}><span className={tone as string} style={{ width: `${100 - index * 14}%` }}><b>{label}</b><strong>{value}</strong></span><small>{index === 0 ? '基准客户' : `${((Number(value) / customerTotal) * 100).toFixed(1)}%`}</small></div>)}</div><div className="funnel-rates"><span>询价率 <b>{funnelRate(funnel.inquiries)}%</b></span><span>样品转化 <b>{funnelRate(funnel.samples)}%</b></span><span>报价转化 <b>{funnelRate(funnel.quotes)}%</b></span><span>整体成交 <b>{conversion}%</b></span></div></section></div><div className="analytics-grid second"><section className="analytics-card"><CardTitle kicker="03 · CUSTOMERS" title="客户分析" action={<button className="analytics-link" onClick={() => navigate('客户 CRM')}>客户 CRM →</button>} /><div className="analysis-numbers"><div><b>{customerTotal}</b><span>客户总数</span></div><div><b>{highIntent}</b><span>高意向</span></div><div><b>{pendingCustomerCount}</b><span>待跟进</span></div><div><b>{longTerm}</b><span>长期客户</span></div></div><div className="bar-list">{typeGroups.map(([label, value]) => <div key={label}><span>{label}</span><i><b style={{ width: `${Number(value) / typeMax * 100}%` }} /></i><strong>{value}</strong></div>)}</div></section><section className="analytics-card"><CardTitle kicker="04 · SOURCES" title="客户来源分析" /><div className="source-table"><div className="source-head"><span>渠道</span><span>客户</span><span>成交</span><span>金额</span></div>{sourceGroups.map(([source, count, done, amount]) => <div key={source}><span>{source}</span><span>{count}</span><span>{done}</span><b>{amount}</b></div>)}</div></section></div><div className="analytics-card product-analysis"><CardTitle kicker="05 · PRODUCTS" title="产品分析" action={<button className="analytics-link" onClick={() => navigate('产品中心')}>产品中心 →</button>} /><div className="product-analysis-table"><div className="product-analysis-head"><span>产品名称</span><span>询价次数</span><span>样品次数</span><span>报价次数</span><span>成交次数</span><span>销售数量</span><span>销售金额</span></div>{productAnalysis.map((row) => <div key={row[0]}><b>{row[0]}</b>{row.slice(1).map((value, index) => <span key={`${row[0]}-${index}`}>{value}</span>)}</div>)}</div></div><div className="analytics-grid third"><section className="analytics-card inventory-analysis"><CardTitle kicker="06 · INVENTORY" title="库存分析" action={<button className="analytics-link" onClick={() => navigate('库存管理')}>库存管理 →</button>} /><div className="inventory-kpis"><div><span>库存总量</span><b>{totalInventory} KG</b></div><div><span>低库存</span><b className="warn">{lowStock}</b></div><div><span>缺货</span><b>{outOfStock}</b></div></div><div className="inventory-risk"><span className="risk-icon">!</span><div><b>{lowStockFirst?.productName ?? '无'}</b><p>当前库存 {lowStockFirst ? lowStockFirst.inbound - lowStockFirst.outbound : 0} KG · 安全库存 {lowStockFirst?.safetyStock ?? 0} KG</p><small>库存偏低，可能影响近期交付。</small></div></div></section><section className="analytics-card quote-analysis"><CardTitle kicker="07 · QUOTES / SAMPLES" title="报价与样品" /><div className="dual-kpis"><div><span>本月报价</span><b>{quoteCount}</b><small>已接受 {acceptedQuotes} · 待确认 {pendingQuotes} · 7天内到期 {expiringQuotes}</small></div><div><span>样品转报价率</span><b>{sampleQuoteRate}%</b><small>{sampleCount} 份样品 · {quoteCount} 份报价</small></div><div><span>样品转订单率</span><b>{sampleOrderRate}%</b><small>{completed} 个已成交</small></div><div><span>等待反馈</span><b className="warn">{pendingSamples}</b><small>建议主动跟进</small></div></div></section></div><div className="analytics-grid last"><section className="analytics-card ranking-card"><CardTitle kicker="08 · TOP CUSTOMERS" title="客户销售排行榜" action={<button className="analytics-link" onClick={() => navigate('客户 CRM')}>客户 CRM →</button>} />{topCustomers.map(({ customer, stats }, index) => <div className="rank-row" key={customer.id}><strong>0{index + 1}</strong><b>{customer.company}</b><span>订单 {stats.orderCount}<small>{money(stats.totalSpent)}</small></span><em>{stats.lastOrderDate || '—'}</em></div>)}</section><section className="analytics-card ranking-card"><CardTitle kicker="09 · TOP PRODUCTS" title="产品销售排行榜" action={<button className="analytics-link" onClick={() => navigate('产品中心')}>产品中心 →</button>} />{topProducts.map((product, index) => <div className="rank-row" key={product.id}><strong>0{index + 1}</strong><b>{product.name}</b><span>{product.quantity} KG<small>{money(product.sales)}</small></span><em>{product.orderCount} 单</em></div>)}</section></div></div>
}
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {activeNav === 'AI 助手' ? <AiAssistant onBack={() => setActiveNav('工作台')} navigate={setActiveNav} /> : activeNav === '数据中心' ? <DataCenter onBack={() => setActiveNav('工作台')} navigate={setActiveNav} /> : activeNav === '产品资料' ? <ProductDocuments onBack={() => setActiveNav('工作台')} /> : activeNav === '配方实验室' ? <FormulaLab onBack={() => setActiveNav('工作台')} /> : activeNav === '订单管理' ? <OrderManagement onBack={() => setActiveNav('工作台')} onCustomer={() => setActiveNav('客户 CRM')} onProduct={() => setActiveNav('产品中心')} /> : activeNav === '报价管理' ? <QuoteManagement onBack={() => setActiveNav('工作台')} onCustomer={() => setActiveNav('客户 CRM')} onProduct={() => setActiveNav('产品中心')} /> : activeNav === '样品管理' ? <SampleManagement onBack={() => setActiveNav('工作台')} /> : activeNav === '客户 CRM' ? <CustomerCRM onBack={() => setActiveNav('工作台')} /> : activeNav === '库存管理' ? <InventoryManagement onBack={() => setActiveNav('工作台')} /> : activeNav === '产品中心' ? <ProductCenter onBack={() => setActiveNav('工作台')} /> : activeNav === 'AI 采集' ? <AICollectionCenter onBack={() => setActiveNav('工作台')} /> : activeNav === '客户跟进' ? <FollowUp onBack={() => setActiveNav('工作台')} /> : activeNav === '系统设置' ? <Settings onBack={() => setActiveNav('工作台')} /> : <div className="page-wrap">
          ...
          <div className="bottom-grid"><section className="panel customer-panel"><div className="section-heading"><div><span className="section-kicker">客户关系</span><h2>客户概况</h2></div><button className="text-button" onClick={() => setActiveNav('客户 CRM')}>客户 CRM <span>→</span></button></div><div className="customer-stats"><div><b>{newCustomers}</b><span>新增客户</span><small>↑ 18.5%</small></div><div><b>{highIntentCustomers}</b><span>高意向客户</span><small>↑ 4.2%</small></div><div><b>{pendingCustomers}</b><span>待跟进客户</span><small className="orange-text">需处理</small></div><div><b>{monthlyClosedCustomers}</b><span>本月成交客户</span><small>↑ 12.5%</small></div></div></section><section className="panel activity-panel"><div className="section-heading"><div><span className="section-kicker">动态记录</span><h2>最近活动</h2></div><button className="text-button">全部动态 <span>→</span></button></div><div className="timeline"><div><time>08-23</time><i /><p><b>XX化妆品</b> 询价阿甘油</p></div><div><time>08-22</time><i /><p>寄出仙人掌籽油样品</p></div><div><time>08-21</time><i /><p>创建阿甘油报价</p></div><div><time>08-20</time><i /><p>新增客户 <b>XX美容院</b></p></div></div></section></div>
        </div>}
      </main>
